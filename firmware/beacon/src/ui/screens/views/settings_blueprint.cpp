// Blueprint / Schematic - SETTINGS. DWG header + dimension rows (label left, value right,
// hairline rules). Theme row taps cycle theme (deferred via lv_async_call - theme_set deletes
// this object mid-event). Brightness row taps cycle 40/60/80/100% (display_brightness inline).
#include "ui/screen.h"
#include "ui/styles.h"
#include "ui/theme.h"
...
// One dimension row: label (display, ink) left, value (mono) right, hairline rule under.
// Returns the value label; optional tap handler makes the whole row clickable.
...
static void build(lv_obj_t* page) {
  ...
  lv_label_set_text(dwg, "DWG. BEACON-006 / SETTINGS");
...
extern const screen_view_t settings_blueprint_view = { build, update };
