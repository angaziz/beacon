...
static void build(lv_obj_t* page) {
  ...
  // Technical header: DWG. BEACON-001 / HOME  ....  SCALE 1:1
  ...
  lv_label_set_text(s_status, "SCALE 1:1");
...
static void update(void) {
  ...
  char buf[32];
  if (sv_status(buf, sizeof(buf), &w.hdr, now)) lv_label_set_text(s_status, buf);
  else lv_label_set_text(s_status, "SCALE 1:1");
