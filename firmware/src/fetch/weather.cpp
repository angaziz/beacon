...
data_err_t fetch_weather(void) {
  ...
  int status = 0;
  data_err_t e = net_https_get("api.open-meteo.com", path, nullptr, nullptr, 0, fetch_scratch(), fetch_scratch_cap(), &status);
  if (e != ERR_NONE) { ds_set_state_weather(e == ERR_NO_ROUTE ? ST_OFFLINE : ST_ERROR, e); return e; }

  weather_rec_t w; memset(&w, 0, sizeof(w));
  e = parse_weather(fetch_scratch(), strlen(fetch_scratch()), &w);
