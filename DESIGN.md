...
6. **PAL** — animated coding-agent mascot (pixel-art sprite, ported from the Clawdmeter project) reflecting Coding Buddy state: asleep (no hub link / no sessions), an idle-animation rotation, bounce + a pulsing alert ring on a pending permission prompt, or a working animation while a session runs. Unlike screens 1–5, PAL is a **single shared view across all 7 themes**, not per-theme authored: the sprite content doesn't change per theme, only its background/alert-ring tint (token-driven, same pattern the gauge component uses). See `docs/specs/2026-07-17-pal-screen-design.md`. [BLE / Mac hub]
