...
### C.6 omp extension (buddy) — VERIFIED (oh-my-pi/pi-coding-agent @ 17.1.4)
...
generic `HookBuddyProvider` as Codex). Request body is byte-shaped like §C.3 minus the tool fields:
`{hook_event_name, session_id, cwd}`. Every omp event is fire-and-forget; the hub answers `{"ok":true}`.
Lifecycle events, mapped from omp extension events: `session_start`/`session_switch`/`session_branch` =>
`SessionStart` (rebinds `session_id` from `ctx.sessionManager.getSessionId()`), `agent_start` =>
`UserPromptSubmit`, `agent_end` (unless `willContinue`) => `Stop`, `session_shutdown` => `SessionEnd`,
`tool_approval_requested` => `Notification`, `tool_approval_resolved` => `ApprovalResolved`. Only
**interactive** sessions report (`ctx.hasUI` on each event itself, not just the cached `session_id` -- a
task subagent runs inside an already-bound interactive session and must be excluded independently; print
mode and subagents both skip, matching Claude).

**omp does NOT gate (CRITICAL, v4, source-verified against 17.1.4).** Unlike Claude Code and Codex, whose
`PermissionRequest` hooks fire *only* when the agent would prompt and whose decision *substitutes* for the
local prompt, omp offers no substitution point:

1. Approval is fully resolved in the tool wrapper **before** extensions see `tool_call`
   (`src/extensibility/extensions/wrapper.ts`: approval at step 1, `emitToolCall` at step 2).
2. `tool_approval_requested` is notify-only -- `ExtensionRunner` maps its handler result to `undefined`
   and the wrapper discards the return, so a handler cannot answer the prompt.
3. The approval prompt's `ExtensionUIContext` is mode-owned (`modes/controllers/extension-ui-controller.ts`),
   with no registration point for an extension to supply or wrap `select`.

So a `tool_call` gate can only ever be a **second** prompt: under a non-yolo `tools.approvalMode` the user
has already answered on the Mac, and under `yolo` (the schema default) it re-asks exactly what the user told
omp not to ask about. v3 gated `bash` this way and was the sole reason a yolo omp session still had to tap
the device. v4 therefore **mirrors** instead: an approval prompt marks that session `question` (device shows
it waiting, tap-to-open focuses the terminal per §C.4/FR-BUDDY-8) and the answer clears it back to working.
Nothing in the extension can block a tool call. Consequence, accepted by the owner (2026-07-27): omp has no
remote approve/deny; Claude Code and Codex keep theirs, since their hooks do substitute.

**Descriptor.** The omp provider registers `capabilities: [.sessions]` -- no `.usage` (omp quota reports
duplicate Claude/Codex) and no `.prompts`. `supportsBuddy` is `sessions || prompts`, so the Coding Buddy
toggle still appears for omp. `HookBuddyProvider.permissionCore` answers `{}` (no verdict) for any provider
lacking `.prompts`, checked *before* the quit-drain, buddy-off and device-offline branches: a stale v3 file
still POSTing `PermissionRequest` stops gating immediately rather than at the next reinstall, and a provider
that never holds a request has nothing to fail closed. `capSeconds` is inert for omp.
...
clearing them on `SessionEnd`. Codex sends none, so its tap-to-open stays a no-op. Extension source marker
is `beacon-omp v4`; detection is exact-content equality, so any v1/v2/v3 file reads as not-current and
...
- **Timing (per provider):** design target < 5 s round-trip. Hold caps differ by caller deadline — Claude/Codex
  ~590 s hold (below CC's/Codex's ~600 s hook timeout; Codex chain `hub 575 < curl 585 < hook 590`). omp does
  not gate at all (§C.6), so it holds nothing. Only a queued prompt's own cap expiry denies it (silently);
  `deny` + label on cap (`tech.md` §8, FR-BUDDY-3).
- **Undeliverable prompt (device offline) => pass-through, never deny.** A prompt that arrives while the
  ...
  menubar alert "Beacon offline - <AGENT> not gated" (cleared on reconnect), for the gating providers
  (claude, codex). Denying a prompt the user never saw fails the tool call with nothing to act on;
  fail-closed applies to prompts the device DID show (cap expiry) and to the quit drain, which still deny.
