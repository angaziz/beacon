...
- **`statusLine.command` hard-codes an absolute author path** (`/Users/<author>/work/personal/beacon/...`) in `claude-code-settings.snippet.json:39`. **Broken out-of-box for anyone but the author**, and the statusline is the *primary* Claude-usage source — so a new user silently gets no Claude usage/context and no error. **P1** (breaks Claude usage/context, not the permission/safety path; **P0 if this is ever distributed beyond the author**).
