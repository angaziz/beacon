...
- **Permission prompts:** `PreToolUse` / `PermissionRequest` **command hooks** (the `beacon-claude-hook` shim curls the hub; a `type:http` hook would print an error line in Claude Code on every event while the hub is down) → hub; hub forwards to device; device decision returns as the hook's `permissionDecision` (`allow`/`deny`). Blocking ⇒ design for <5 s; on timeout treat as **deny** and label; if the user answers on the Mac (held hook closed), the hub withdraws the device prompt silently (§7.1).
