...
>
> **Status (2026-06-07):** research, design system, and hardware spikes (display/power + WiFi/BLE coexistence) are done and proven. **P0 is partially built** and running on-device (in `firmware/beacon/`): bring-up, the frozen shared contracts + theme engine, and the swipe carousel with all six screens rendered bespoke-per-theme (42 views). **Remaining in P0:** NVS persistence (FR-PLAT-3), WiFi provisioning (FR-SET-1), time service (FR-PLAT-8), idle dim/sleep (FR-PLAT-7), and live Settings actions. Conventions: requirement IDs are stable; priority is MoSCoW (MUST/SHOULD/COULD); each requirement has a Phase (see §7).
...
- **NimBLE vs Bluedroid for the hub link**: Bluedroid proven (P2 default); revisit only if RAM pressure appears.
