...
**P2 progress (2026-06-08):** code complete and running on hardware. **Device:** `core/hub_proto` Arduino-free codec (+ host tests), `core/hublink_ble` NUS peripheral (bonded LESC, NimBLE-backed core `BLE*` wrapper), `core/hub_task` Core-0 wiring (frame => `ds_set_usage`/`ds_set_buddy`, edge-triggered `ds_set_hub_offline`), 7 buddy decide-paths wired to `hub_send_permission` (send-gated clear), Settings pair overlay. **Hub (`hub/`, SwiftPM menubar app):** `BeaconCentral` (CoreBluetooth NUS central, OS-mediated pairing, acked `.withResponse` RX writes, auto-reconnect), `UsagePoller` (Claude Keychain + Codex `~/.codex`), `ClaudeCodeBridge` (blocking permission hooks + idle/session + statusline shim), `MenubarController`, runs as a signed `.app`. **Live on-device:** AI Usage (Codex via `~/.codex`; Claude via the Claude Code statusline `rate_limits`, since the unofficial `oauth/usage` now 429s) over BLE while the device-direct WiFi/TLS plane runs simultaneously; **heap re-measured** under the full load => LVGL draw buffers moved to PSRAM (`-DBEACON_LVGL_PSRAM`, now the default build flag; `tech.md` §2). **Validated on-device:** buddy permission round-trip (FR-BUDDY-2/3) — approve/deny honored on the Mac. **Last item:** P2-0 contract capture — replace the §C draft shapes in `hub/CONTRACT.md` with real token-redacted captures so the device/hub fixtures pin to ground truth. **Scope cut:** device-initiated *launch* (FR-BUDDY-4 / FR-HUB-4) removed.

**P3 progress (2026-06-10):** code complete + host-tested; on-device gesture/threshold tuning pending. Pure decision logic split out Arduino/LVGL-free and host-tested under `[env:native]`: `core/idle_policy.h` (idle power level) + `core/motion_gesture.{h,cpp}` (jerk-peak flick/shake detector) with `test_idle_policy` + `test_motion_gesture`. **Device:** `hal/imu.{cpp,h}` (QMI8658 accel via SensorLib on the shared Wire bus), `hal/display.display_set_on` (DCS 0x28/0x29 panel sleep), `ui/input.{cpp,h}` Core-1 controller — idle dim (30s) -> sleep (5min) via `lv_disp_get_inactive_time`, wake on touch (indev gate swallows the waking tap) + IMU flick/raise, shake => dismiss topmost overlay (FR-PLAT-6); `ui/brightness_overlay.{cpp,h}` swipe-down quick brightness (FR-PLAT-5); long-press => per-screen `screen_module_t.context_action` (Home/Finance => `fetch_task_refresh_now`). Closes **FR-PLAT-5/6** and the deferred **FR-PLAT-7** (idle dim/sleep). `pio run -e beacon` clean (60.5% of app partition); all 94 host tests green. **Pending (owner):** confirm the QMI8658 I2C address (0x6B vs 0x6A) and tune the gesture/idle thresholds on hardware.
