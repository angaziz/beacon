...
> discoverable tap → Wi-Fi panel, and adds multi-network support. Conventions: `docs/tech.md` §6/§9/§10.
...
- **Adding a network:** only via the `Beacon-setup` SoftAP portal on the phone (no on-screen keyboard).
  The portal **appends** to a saved list instead of overwriting.
...
| `core/provision.cpp` | Portal **appends** via the list API (replaces the `nvs_set_wifi` overwrite at the save handler). If the list is full, the portal returns a "list full — forget one on the device" message instead of saving. Launched by first-boot (empty list) or the touch-hold boot hatch (already built). |
| `ui/wifi_panel.{h,cpp}` | **One shared** modal overlay on `lv_layer_top`, fully **self-contained** (references NO carousel page object — those are destroyed by `on_theme`/`lv_obj_clean`), styled from `theme_active()` tokens, built once. `wifi_panel_open()` + internal close. Renders status + scrollable saved list + Connect/Disconnect toggle + ‹ Back; tapping a row opens an in-overlay **Forget "X"?** confirm. (Theme can't change while open — the theme picker lives in Settings, which the panel covers — so stale styling isn't reachable.) |
...
- **Forget last / empty list:** offline now; next boot has empty list ⇒ `provision_needed()` ⇒ portal.
  Re-add live via the boot hatch portal.
...
- **On-device (human) acceptance:** tap Wi-Fi row opens the panel in every theme; add a 2nd network via
  the portal and confirm both auto-join while moving between them; forget (incl. the active one)
  behaves; Disconnect/Connect toggles and Connect re-joins; Back always restores swipe; boot hatch
  still forces the portal; reconnect roams to the strongest saved net.
