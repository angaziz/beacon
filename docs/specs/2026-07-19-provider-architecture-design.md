...
- Installer: managed block in `~/.codex/config.toml` `[hooks]` wiring the five events to the shim; verify the exact TOML matcher-group shape against `openai/codex` `codex-rs/config/src/hook_config.rs` before writing. Detection mirrors `HooksDetection`.
